        ...
        return (
            <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#2a1b1b] to-black relative overflow-hidden flex items-center justify-center p-4">
                ...
                <div className="w-full max-w-3xl relative z-10">
                    <h1 className="text-3xl font-serif font-bold mb-8 text-center text-transparent bg-clip-text bg-gradient-to-r from-red-200 to-rose-100 drop-shadow-sm">Welcome! Let's get you set up.</h1>
        ...
        <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#2a1b1b] to-black relative overflow-hidden text-white">
            ...
            <div className="container mx-auto p-6 relative z-10">
                <header className="flex justify-between items-center mb-10 border-b border-white/10 pb-6">
                    <div>
                        <h1 className="text-3xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-200 to-rose-100">{household.name}</h1>
                        <p className="text-red-200/60 font-medium">Registered with {email}</p>
                    </div>
                    <Link href="/api/auth/logout" className="text-sm text-red-400 hover:text-red-300 transition-colors uppercase tracking-wider font-semibold">Sign Out</Link>
                </header>

                <section className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-8 shadow-xl mb-8">
                    <h2 className="text-xl font-serif text-white/90 mb-6">Your Status</h2>
                    ...
                    <div className="mt-8">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {household.kids.map((kid: { id: string; name: string }) => (
                                <div key={kid.id} className="bg-black/20 border border-white/5 rounded-xl p-4 flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-linear-to-br from-red-500 to-rose-600 flex items-center justify-center text-xs font-bold shadow-lg">
                                        {kid.name.charAt(0)}
                                    </div>
